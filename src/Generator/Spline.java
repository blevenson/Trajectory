...
public class Spline {
	...
	public static boolean reticulateSplines(double x0, double y0,
			double theta0, double x1, double y1, double theta1, Spline result,
			Type type) {
		...
		double theta0_hat = getDifferenceInAngleRadians(
				result.theta_offset_, theta0);
		double theta1_hat = getDifferenceInAngleRadians(
		...
		if (Math.abs(getDifferenceInAngleRadians(theta0_hat,
				theta1_hat)) >= Math.PI / 2) {
			return false;
		}
		...
		if (type == CubicHermite) {
			...
			result.a_ = -(3 * (yp0_hat + yp1_hat))
					/ (x1_hat * x1_hat * x1_hat * x1_hat);
			result.b_ = (8 * yp0_hat + 7 * yp1_hat)
	...
	public double calculateLength() {
		...
		double arc_length = 0;
		double t, dydt;
		double integrand, last_integrand = Math.sqrt(1 + derivativeAt(0)
				* derivativeAt(0))
				/ kNumSamples;
	...
	public double[] getXandY(double percentage) {
		...
		double y_hat = (a_ * x_hat + b_) * x_hat * x_hat * x_hat * x_hat + c_
				* x_hat * x_hat * x_hat + d_ * x_hat * x_hat + e_ * x_hat;
	...
	private double derivativeAt(double percentage) {
		...
		double x_hat = percentage * knot_distance_;
		double yp_hat = (5 * a_ * x_hat + 4 * b_) * x_hat * x_hat * x_hat + 3
				* c_ * x_hat * x_hat + 2 * d_ * x_hat + e_;
	...
	private double secondDerivativeAt(double percentage) {
		...
		double x_hat = percentage * knot_distance_;
		double ypp_hat = (20 * a_ * x_hat + 12 * b_) * x_hat * x_hat + 6 * c_
				* x_hat + 2 * d_;
