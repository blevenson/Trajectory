...
public class TrajectoryGenerator {
	...
	// currently supported.
	public static final Strategy SCurvesStrategy = new Strategy(
			"SCurvesStrategy");

	// Choose one of the above strategies based on the inputs.
	public static final Strategy AutomaticStrategy = new Strategy(
			"AutomaticStrategy");
	 ...
	 *         conditions.
	 */
	public static Trajectory generate(Config config, Strategy strategy,
			double start_vel, double start_heading, double goal_pos,
		...
		if (strategy == StepStrategy) {
			...
			double impulse = (goal_pos / adjusted_max_vel) / config.dt
					- start_vel / config.max_acc / config.dt + start_discount
					+ end_discount;
			...
			double adjusted_max_vel = Math.min(
					config.max_vel,
					(-config.max_acc * config.max_acc + Math
							.sqrt(config.max_acc * config.max_acc
									* config.max_acc * config.max_acc + 4
									* config.max_jerk * config.max_jerk
									* config.max_acc * goal_pos))
							/ (2 * config.max_jerk));

			// Compute the length of the linear filters and impulse.
			int f1_length = (int) Math.ceil((adjusted_max_vel / config.max_acc)
					/ config.dt);
			int f2_length = (int) Math.ceil((config.max_acc / config.max_jerk)
					/ config.dt);
			double impulse = (goal_pos / adjusted_max_vel) / config.dt;
	...
	private static Trajectory secondOrderFilter(int f1_length, int f2_length,
		...
		for (int i = 0; i < length; ++i) {
			...
			if (integration == RectangularIntegration) {
				...
				traj.segments_[i].pos = (last.vel + traj.segments_[i].vel)
						/ 2.0 * dt + last.pos;
			}
